  ...
  return (
    <>
      <div className={`navbar-wrapper ${scroll ? 'nav-scroll' : ''}`}>
        <div className={`hamburger ${toggle ? "toggle" : ""}`} onClick={ontoggleHanle}>
          <div className={`line one`}></div>
          <div className={`line two`}></div>
          <div className={`line three`}></div>
          <div className={`line four`}></div>
        </div>
        <div className="px-md-5 p-4" style={{ opacity: `${scrollFar ? 1 : 0}`, transition: '0.75s' }}>
        ...
        <ul className={`nav-links ${toggle ? "open" : ""}`}>
          ...
          <li class="resume">
            <a href={resume} target="_blank" className="quote text-decoration-none">RESUME</a>
          </li>
        </ul>
      </div>
    </>
